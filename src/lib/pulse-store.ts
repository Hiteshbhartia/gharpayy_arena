...
export const SLOTS: SlotDef[] = [
  { key: "slot1", label: "Slot 1 · Morning Pulse",   window: "10:30 – 1:15",  startMin: 10 * 60 + 30, endMin: 13 * 60 + 15, prompt: "What did you ship between 10:30 and 1:15? Calls, tours, follow-ups, blockers." },
  { key: "slot2", label: "Slot 2 · Afternoon Pulse", window: "2:00 – 5:00",   startMin: 14 * 60,      endMin: 17 * 60,      prompt: "What moved between 2:00 and 5:00? Closures, escalations, on-ground wins." },
  { key: "slot3", label: "Slot 3 · Evening Pulse",   window: "5:20 – 8:00",   startMin: 17 * 60 + 20, endMin: 20 * 60,       prompt: "Final stretch 5:20 – 8:00. Conversions, callbacks set for tomorrow, tickets cleared." },
  { key: "eod",   label: "End of Day Brief",         window: "By 8:30",       startMin: 0,            endMin: 20 * 60 + 30,  prompt: "EOD: numbers for the day, what you closed, what's pending, what you need help on." },
];
...
export function getEntries(filter?: { employeeId?: string; date?: string; slot?: SlotKey }): PulseEntry[] {
  let rows = load();
...
export function submitPulse(input: {
  ...
  const date = todayISO();
  const rows = load();
  const existingIdx = rows.findIndex((r) => r.employeeId === input.employeeId && r.date === date && r.slot === input.slot);
  ...
  const entry: PulseEntry = {
    id: existingIdx >= 0 ? rows[existingIdx].id : `p_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`,
...
export function complianceFor(employeeId: string, date = todayISO()): { done: SlotKey[]; missing: SlotKey[] } {
...
export function orgComplianceToday(): { employeeId: string; name: string; team: string; role: string; done: SlotKey[]; missing: SlotKey[]; rate: number }[] {
  const date = todayISO();
  return getRoster()
    ...
    .map((e) => {
      ...
      const m = nowMinutes();
      const expected = SLOTS.filter((s) => m >= s.endMin || s.key === "eod" && m >= 20 * 60).map((s) => s.key);
      ...
      return { employeeId: e.id, name: e.name, team: e.team, role: e.role, done: c.done, missing: c.missing, rate };
    })
