...
export const Route = createFileRoute("/hrms")({
  component: () => <RoleGate allow={["leadership", "hr"]}><HrmsPage /></RoleGate>,
  ...
  return (
    <div className="px-4 md:px-8 py-6 md:py-8 max-w-[1400px] mx-auto">
      <header className="mb-6">
        <div className="font-mono text-[11px] uppercase tracking-widest text-primary mb-1.5">People Operations</div>
        <h1 className="font-display text-2xl md:text-4xl font-semibold tracking-tight">HRMS Dashboard</h1>
        <p className="text-muted-foreground text-sm mt-1">Roles, permissions and people — one source of truth.</p>
      </header>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <span className={`inline-flex items-center gap-1.5 text-[10px] font-mono uppercase tracking-widest px-2 py-0.5 rounded border ${ROLE_TONE[r]}`}>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-5 gap-4 mb-6">
        <div className="lg:col-span-3 rounded-2xl bg-card border border-border overflow-hidden">
          <div className="px-4 md:px-5 py-3 md:py-4 border-b border-border flex items-center justify-between">
            <div>
              <h2 className="font-display text-base md:text-lg font-semibold">{ROLE_LABEL[activeRole]} roster</h2>
              <p className="text-xs text-muted-foreground">{grouped[activeRole].length} people · live presence</p>
            </div>
            <Link to="/people" className="text-xs text-primary hover:underline hidden sm:inline">All people →</Link>
          </div>
              ...
              return (
                <div key={e.id} className="px-4 md:px-5 py-3 flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[11px] text-muted-foreground truncate">{e.role} · {e.team}</div>
                  </div>
        ...
        <div className="lg:col-span-2 rounded-2xl bg-card border border-border overflow-hidden">
          <div className="px-4 md:px-5 py-3 md:py-4 border-b border-border">
            <h2 className="font-display text-base md:text-lg font-semibold">Permissions for {ROLE_LABEL[activeRole]}</h2>
          ...
          <div className="divide-y divide-border">
            {groupedCaps.map(([group, caps]) => (
              <div key={group} className="px-4 md:px-5 py-3">
                <div className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground mb-2">{group}</div>
                  ...
                  {caps.map((c) => {
                    ...
                    return (
                      <li key={c.cap} className="flex items-center gap-2 text-sm">
                        ...
                        <span className={allowed ? "text-foreground" : "text-muted-foreground line-through"}>{c.label}</span>
                      </li>
                    );
                  })}
                </ul>
      ...
      <section className="rounded-2xl bg-card border border-border overflow-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 p-4 md:p-5">
          <ActionCard to="/roster" title="Live roster" sub="Who's where, right now" icon={Users} show={can(activeRole, "view_all_attendance")} />
          <ActionCard to="/leaves" title="Approve leaves" sub="Clear the queue" icon={ShieldCheck} show={can(activeRole, "approve_leaves")} />
          <ActionCard to="/tasks" title="Assign tasks" sub="Move the team forward" icon={UserCog} show={can(activeRole, "assign_tasks_team")} />
          <ActionCard to="/score" title="Your score" sub="See where you stand" icon={User} show={can(activeRole, "view_own_score")} />
          <ActionCard to="/war-room" title="War Room" sub="Daily snapshot" icon={ShieldCheck} show={can(activeRole, "view_war_room")} />
          <ActionCard to="/achievements" title="Achievements" sub="Recognition wall" icon={ShieldCheck} show />
        </div>
      </section>
    </div>
  );
...
function ActionCard({ to, title, sub, icon: Icon, show }: { to: string; title: string; sub: string; icon: React.ElementType; show: boolean }) {
  if (!show) return null;
  return (
    <Link to={to} className="group flex items-center gap-3 rounded-xl border border-border bg-background hover:border-primary/40 hover:bg-primary/5 transition-colors p-3">
