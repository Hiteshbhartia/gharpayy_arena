...
import {
  ArrowDown, ArrowUp, Flame, Target, IndianRupee, Phone, TrendingUp, AlertTriangle,
  CheckSquare, Calendar as CalIcon, PlaneTakeoff, Users, Cake, Trophy, Clock, Square, CheckCircle2,
} from "lucide-react";
...
function PillarHeader({ kicker, title, href, icon: Icon }: { kicker: string; title: string; href: string; icon: React.ElementType }) {
  return (
    <div className="flex items-center justify-between mb-3">
      <div className="flex items-center gap-2">
        <Icon className="h-4 w-4 text-primary" />
        <div>
          <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{kicker}</div>
          <h2 className="font-display text-lg font-semibold leading-tight">{title}</h2>
        </div>
      </div>
      <Link to={href} className="text-[11px] text-primary font-mono uppercase tracking-widest">Open →</Link>
    </div>
  );
}
...
function TimePillar({ actor }: { actor: Employee }) {
  ...
  const statusColor =
    status === "Clocked In" ? "text-success"
    : status === "On Break" ? "text-warning"
    : status === "In Field" ? "text-primary"
  ...
  return (
    <section className="rounded-xl bg-card border border-border p-4 md:p-5">
      ...
      <div className="flex items-baseline justify-between mb-3">
        <div className="font-display text-2xl md:text-3xl font-semibold tabular-nums">{fmtDuration(summary.workMs)}</div>
        <div className={`font-mono text-[10px] uppercase tracking-widest ${statusColor}`}>{status}</div>
      </div>
      <div className="grid grid-cols-3 gap-2 text-center mb-4">
        <div className="rounded-md bg-muted/40 border border-border py-2">
          <div className="text-[9px] font-mono uppercase tracking-widest text-muted-foreground">Work</div>
          <div className="text-sm font-semibold tabular-nums">{fmtDuration(summary.workMs)}</div>
        </div>
        <div className="rounded-md bg-muted/40 border border-border py-2">
          <div className="text-[9px] font-mono uppercase tracking-widest text-muted-foreground">Break</div>
          <div className="text-sm font-semibold tabular-nums">{fmtDuration(summary.breakMs)}</div>
        </div>
        <div className="rounded-md bg-muted/40 border border-border py-2">
          <div className="text-[9px] font-mono uppercase tracking-widest text-muted-foreground">Field</div>
          <div className="text-sm font-semibold tabular-nums">{fmtDuration(summary.fieldMs)}</div>
        </div>
      </div>
      <Link to="/attendance" className="block w-full text-center py-2.5 rounded-md bg-primary text-primary-foreground text-sm font-medium">
...
function TasksPillar({ actor }: { actor: Employee }) {
  ...
  const nextWithSub = open.find((t) => (t.subtasks?.length ?? 0) > 0 && t.subtasks!.some((s) => !s.done));
  ...
  return (
    <section className="rounded-xl bg-card border border-border p-4 md:p-5">
      ...
      <div className="flex items-baseline justify-between mb-2">
        <div className="font-display text-2xl md:text-3xl font-semibold tabular-nums">{open.length}<span className="text-base text-muted-foreground font-normal"> open</span></div>
        <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{pct}% done</div>
      </div>
      <Progress value={pct} className="h-1.5 mb-4" />
      <div className="grid grid-cols-3 gap-2 text-center mb-4">
        <div className="rounded-md bg-muted/40 border border-border py-2">
          <div className="text-[9px] font-mono uppercase tracking-widest text-muted-foreground">To do</div>
          <div className="text-sm font-semibold tabular-nums">{todo}</div>
        </div>
        <div className="rounded-md bg-muted/40 border border-border py-2">
          <div className="text-[9px] font-mono uppercase tracking-widest text-muted-foreground">Doing</div>
          <div className="text-sm font-semibold tabular-nums">{doing}</div>
        </div>
        <div className="rounded-md bg-muted/40 border border-border py-2">
          <div className="text-[9px] font-mono uppercase tracking-widest text-muted-foreground">Done</div>
      ...
      {nextWithSub && nextSub ? (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground truncate">{nextWithSub.title}</div>
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground">Next up</div>
...
function GoalsPillar({ actor }: { actor: Employee }) {
  ...
  return (
    <section className="rounded-xl bg-card border border-border p-4 md:p-5">
      ...
      <div className="flex items-baseline justify-between mb-3">
        <div className="font-display text-2xl md:text-3xl font-semibold tabular-nums">{score.total}<span className="text-base text-muted-foreground font-normal">/100</span></div>
        <span className={`inline-flex items-center justify-center h-7 w-7 rounded-md border font-mono font-semibold text-xs ${tierColor[tier]}`}>{tier}</span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded-md bg-muted/40 border border-border p-2">
          <div className="text-[9px] font-mono uppercase tracking-widest text-muted-foreground">Attendance</div>
        ...
        <div className="rounded-md bg-muted/40 border border-border p-2">
          <div className="text-[9px] font-mono uppercase tracking-widest text-muted-foreground">On-time tasks</div>
          <div className="flex items-center justify-between mt-0.5">
        ...
        <div className="rounded-md bg-muted/40 border border-border p-2">
          <div className="text-[9px] font-mono uppercase tracking-widest text-muted-foreground">Role KPI</div>
          <div className="flex items-center justify-between mt-0.5">
        ...
        <div className="rounded-md bg-muted/40 border border-border p-2">
          <div className="text-[9px] font-mono uppercase tracking-widest text-muted-foreground">Kudos</div>
          <div className="flex items-center justify-between mt-0.5">
...
function PodPillars({ pod, label }: { pod: Employee[]; label: string }) {
  const stats = useMemo(() => {
    ...
    for (const e of pod) {
      ...
      doneToday += ts.filter((t) => t.status === "done" && (t.completedAt ?? 0) >= startOfDay.getTime()).length;
  ...
  return (
    <section className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4 mb-6">
      <div className="rounded-xl bg-card border border-border p-4 md:p-5">
        <PillarHeader kicker={`${label} · Pillar 01`} title="Time on the floor" href="/attendance" icon={Clock} />
        <div className="font-display text-2xl md:text-3xl font-semibold tabular-nums">{stats.onClock + stats.inField}<span className="text-base text-muted-foreground font-normal">/{stats.total}</span></div>
        ...
        <div className="flex gap-2 text-[10px] font-mono uppercase tracking-widest">
          ...
          <span className="px-2 py-1 rounded bg-warning/15 text-warning">Break {stats.onBreak}</span>
          <span className="px-2 py-1 rounded bg-primary/10 text-primary">Field {stats.inField}</span>
        </div>
      </div>

      <div className="rounded-xl bg-card border border-border p-4 md:p-5">
        <PillarHeader kicker={`${label} · Pillar 02`} title="Task throughput" href="/tasks" icon={CheckSquare} />
        <div className="font-display text-2xl md:text-3xl font-semibold tabular-nums">{stats.doneToday}</div>
        <div className="text-xs text-muted-foreground mt-1 mb-3">completed today · {stats.openTasks} still open</div>
        <Bar value={stats.doneToday + stats.openTasks > 0 ? (stats.doneToday / (stats.doneToday + stats.openTasks)) * 100 : 0} color="bg-info" />
      </div>

      <div className="rounded-xl bg-card border border-border p-4 md:p-5">
        <PillarHeader kicker={`${label} · Pillar 03`} title="Goal attainment" href="/score" icon={Trophy} />
        <div className="font-display text-2xl md:text-3xl font-semibold tabular-nums">{stats.avg}<span className="text-base text-muted-foreground font-normal">/100</span></div>
        <div className="text-xs text-muted-foreground mt-1 mb-3">avg score · {stats.atRisk} below 60</div>
        <Bar value={stats.avg} color={stats.avg > 75 ? "bg-success" : stats.avg > 60 ? "bg-info" : "bg-warning"} />
      </div>
    </section>
  );
}
...
function TeammateHome({ actor }: { actor: Employee }) {
  ...
  const todayEvents = useMemo(
    () => events.filter((e) => (e.withIds?.includes(actor.id) || e.ownerId === actor.id) && new Date(e.startAt).toDateString() === new Date().toDateString()),
    [events, actor.id]
  );
  const notifs = useNotifications(actor.id).filter((n) => !n.read).slice(0, 3);
  const myTasks = tasksFor(actor.id).filter((t) => t.status !== "done").slice(0, 4);

  return (
    <div className="px-4 md:px-8 py-6 md:py-8 max-w-[1200px] mx-auto">
      <HomeHeader actor={actor} sub={`Time, Tasks, Goals — your three pillars. ${TIER_TAGLINE.teammate}`} />
      <MissionBrief actor={actor} />
      <HeroPillars actor={actor} />

      <div className="grid md:grid-cols-2 gap-4">
        <section className="rounded-xl bg-card border border-border p-5">
          <PillarHeader kicker="Detail" title="Today's task list" href="/tasks" icon={CheckSquare} />
          {myTasks.map((t) => (
            <div key={t.id} className="py-2 border-b border-border last:border-0">
              ...
              <div className="text-[11px] text-muted-foreground font-mono uppercase">
                {t.priority} · due {new Date(t.dueAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
              </div>
            </div>
          ))}
          {myTasks.length === 0 && <div className="text-sm text-muted-foreground">All clear. Pick up a kudo or coach a teammate.</div>}
        </section>

        <section className="rounded-xl bg-card border border-border p-5">
          <PillarHeader kicker="Detail" title="Heads up" href="/inbox" icon={CalIcon} />
          {notifs.length === 0 && <div className="text-sm text-muted-foreground">Quiet inbox. Nice.</div>}
          {notifs.map((n) => (
          ...
          {todayEvents.length > 0 && (
            <div className="mt-3 pt-3 border-t border-border">
              <div className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground mb-1.5">Today on your calendar</div>
              {todayEvents.map((e) => (
                <div key={e.id} className="text-sm">{e.title} <span className="text-muted-foreground">· {new Date(e.startAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span></div>
              ))}
            </div>
          )}
        </section>
...
function LeaderHome({ actor }: { actor: Employee }) {
  const pod = useMemo(
    () => getRoster().filter((e) => e.managerId === actor.id || e.team === actor.team).filter((e) => e.id !== actor.id),
    [actor.id, actor.team]
  );
  const leaves = useLeaves().filter((l) => l.status === "pending" && pod.some((p) => p.id === l.employeeId));
  ...
  return (
    <div className="px-4 md:px-8 py-6 md:py-8 max-w-[1300px] mx-auto">
      <HomeHeader actor={actor} sub={`Your pod's Time, Tasks, Goals at a glance. ${TIER_TAGLINE.leader}`} />
      <MissionBrief actor={actor} />
      ...
      <section className="rounded-xl bg-card border border-border overflow-hidden">
        <div className="px-4 md:px-5 py-3 md:py-4 border-b border-border flex items-center justify-between">
          ...
          <Link to="/war-room" className="text-xs text-primary font-mono uppercase tracking-widest">War Room →</Link>
        </div>
          ...
          {sorted.map((e, i) => {
            ...
            return (
              <div key={e.id} className="px-4 md:px-5 py-3 flex items-center gap-3">
                ...
                <span className={`hidden sm:inline-flex items-center justify-center h-7 w-7 rounded-md border font-mono font-semibold text-xs ${tierColor[t]}`}>{t}</span>
                <div className="w-24 hidden md:block"><Bar value={sc.total} color={sc.total > 85 ? "bg-success" : sc.total > 70 ? "bg-info" : sc.total > 55 ? "bg-warning" : "bg-destructive"} /></div>
                <div className="font-mono text-sm w-10 text-right">{sc.total}</div>
              </div>
            );
          })}
          {sorted.length === 0 && <div className="px-5 py-6 text-sm text-muted-foreground">No teammates assigned to your pod yet.</div>}
        </div>
      </section>
      ...
      {leaves.length > 0 && (
        <section className="mt-6 rounded-xl bg-card border border-border p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link to="/leaves" className="text-xs text-primary font-mono uppercase tracking-widest">Open queue →</Link>
          </div>
          {leaves.slice(0, 4).map((l) => {
            const emp = getRoster().find((e) => e.id === l.employeeId);
            return (
              <div key={l.id} className="py-2 border-b border-border last:border-0 text-sm">
                <span className="font-medium">{emp?.name}</span> · {l.type} · {l.startDate}{l.startDate !== l.endDate && ` → ${l.endDate}`}
...
function HRHome({ actor }: { actor: Employee }) {
  ...
  const lowAttendance = getRoster().filter((e) => e.attendance < 80).sort((a, b) => a.attendance - b.attendance);
    ...
    <div className="px-4 md:px-8 py-6 md:py-8 max-w-[1300px] mx-auto">
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6">
        <div className="rounded-xl bg-card border border-border p-4">
          <div className="flex items-center justify-between"><span className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">Headcount</span><Users className="h-4 w-4 text-primary" /></div>
          <div className="mt-2 font-display text-2xl font-semibold">{getRoster().length}</div>
          <div className="mt-1 text-[11px] text-muted-foreground">{newJoiners.length} new this year</div>
        </div>
        <div className="rounded-xl bg-card border border-border p-4">
          <div className="flex items-center justify-between"><span className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">Leaves Pending</span><PlaneTakeoff className="h-4 w-4 text-warning" /></div>
        ...
        <div className="rounded-xl bg-card border border-border p-4">
          <div className="flex items-center justify-between"><span className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">Attendance Risk</span><AlertTriangle className="h-4 w-4 text-destructive" /></div>
        ...
        <div className="rounded-xl bg-card border border-border p-4">
          <div className="flex items-center justify-between"><span className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">Birthdays</span><Cake className="h-4 w-4 text-info" /></div>
          <div className="mt-2 font-display text-2xl font-semibold">{birthdays.length}</div>
          <div className="mt-1 text-[11px] text-muted-foreground truncate">{birthdays.map(b=>b.name.split(" ")[0]).join(", ") || "—"}</div>
        </div>
      </section>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <section className="rounded-xl bg-card border border-border p-5">
          ...
          {leaves.length === 0 && <div className="text-sm text-muted-foreground">Inbox zero. Beautiful.</div>}
        ...
        <section className="rounded-xl bg-card border border-border p-5">
          ...
          {flagged.slice(0, 5).map((e) => (
            <div key={e.id} className="py-2 border-b border-border last:border-0 flex items-center gap-3">
              <Avatar id={e.id} size={28} />
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium truncate">{e.name} <span className="text-muted-foreground font-normal">· {e.role}</span></div>
                <div className="text-[11px] text-destructive">{e.flags.join(" · ")}</div>
              </div>
              <span className="font-mono text-xs">{e.attendance}%</span>
            </div>
          ))}
          {flagged.length === 0 && <div className="text-sm text-muted-foreground">No flags raised. Send a kudo today.</div>}
        </section>
      </div>
    </div>
  );
...
function LeadershipHome({ actor }: { actor: Employee }) {
  ...
  return (
    <div className="px-4 md:px-8 py-6 md:py-8 max-w-[1400px] mx-auto">
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6">
        <div className="rounded-xl bg-card border border-border p-4">
          <div className="flex items-center justify-between"><span className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">Revenue Today</span><IndianRupee className="h-4 w-4 text-primary" /></div>
        ...
        <div className="rounded-xl bg-card border border-border p-4">
          <div className="flex items-center justify-between"><span className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">A-Players</span><Flame className="h-4 w-4 text-success" /></div>
          <div className="mt-2 font-display text-2xl font-semibold">{s.counts.A}</div>
          <div className="mt-1 text-[11px] text-muted-foreground">B:{s.counts.B} C:{s.counts.C} D:{s.counts.D}</div>
        </div>
      </section>
      ...
      <section className="grid md:grid-cols-3 gap-4 mb-6">
        <div className="rounded-xl bg-card border border-border p-5">
          <div className="flex items-center gap-2 text-success mb-3">
            <ArrowUp className="h-4 w-4" /><span className="text-xs font-mono uppercase tracking-widest">Top Performer</span>
          </div>
          <div className="font-display text-xl font-semibold">{s.top?.name ?? "—"}</div>
          <div className="text-xs text-muted-foreground mb-3">{s.top ? `${s.top.role} · ${inr(s.top.revenueImpact)}` : ""}</div>
          {s.top && <Bar value={s.top.performance} color="bg-success" />}
        </div>
        <div className="rounded-xl bg-card border border-border p-5">
          <div className="flex items-center gap-2 text-destructive mb-3">
            <ArrowDown className="h-4 w-4" /><span className="text-xs font-mono uppercase tracking-widest">Replace Signal</span>
          </div>
        ...
        <div className="rounded-xl bg-sidebar text-sidebar-foreground border border-sidebar-border p-5">
          <div className="flex items-center gap-2 text-primary mb-3">
            <TrendingUp className="h-4 w-4" /><span className="text-xs font-mono uppercase tracking-widest">Today's Call</span>
          </div>
          <p className="text-sm text-white leading-relaxed">
            Floor needs <span className="text-primary font-semibold">{Math.max(0, 200 - s.totalCalls)}</span> more calls to hit shift target. Reassign Devansh's pool to Karan.
          </p>
          <Link to="/war-room" className="mt-4 inline-block text-xs font-mono uppercase tracking-widest text-primary">Open War Room →</Link>
        </div>
      </section>
      ...
      <section className="rounded-xl bg-card border border-border overflow-hidden">
        <div className="px-5 py-4 border-b border-border flex items-center justify-between">
          ...
          <Link to="/people" className="text-xs text-primary font-mono uppercase tracking-widest">All people →</Link>
        </div>
        <div className="divide-y divide-border">
