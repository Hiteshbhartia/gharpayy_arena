  ...
  return (
    <div className="px-4 md:px-8 py-6 md:py-8 max-w-[800px] mx-auto">
      <header className="mb-5">
        <div className="font-mono text-[11px] uppercase tracking-widest text-primary mb-1.5">Preferences</div>
      ...
      <div className="rounded-2xl bg-card border border-border p-4 md:p-5 mb-4 flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-xs text-muted-foreground">{actor.role} · {actor.team}</div>
          {user?.email && <div className="text-xs text-muted-foreground mt-0.5">{user.email}</div>}
        </div>
        <Link to="/score" className="text-xs text-primary hover:underline">My score →</Link>
      </div>
      ...
      <div className="rounded-2xl bg-card border border-border divide-y divide-border mb-4">
        ...
        <Toggle icon={Calendar} title="Calendar reminders" sub="15 min before any event" defaultOn />
        <Toggle icon={Shield} title="Selfie + GPS for clock-in" sub="Required for attendance score" defaultOn locked />
...
function Toggle({ icon: Icon, title, sub, defaultOn, locked }: { icon: React.ElementType; title: string; sub: string; defaultOn?: boolean; locked?: boolean }) {
  return (
    <label className="px-4 md:px-5 py-3 flex items-center gap-3 cursor-pointer">
      ...
      <input type="checkbox" defaultChecked={defaultOn} disabled={locked} className="h-4 w-8 appearance-none rounded-full bg-muted checked:bg-primary transition-colors relative cursor-pointer disabled:opacity-50 before:absolute before:top-0.5 before:left-0.5 before:h-3 before:w-3 before:rounded-full before:bg-card before:transition-transform checked:before:translate-x-4" />
    </label>
  );
}
