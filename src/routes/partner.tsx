...
import {
  Building2, Wallet, MessageSquareWarning, CalendarDays,
  TrendingUp, TrendingDown, IndianRupee, Star, Phone, MapPin, Plus, Send,
  CheckCircle2, Clock, AlertTriangle, FileText,
} from "lucide-react";
import {
  PROPERTIES, PARTNER_TICKETS, PARTNER_PAYOUTS, ZONES,
  propertiesOfPartner, payoutsOfPartner, ticketsOfPartner, occPct, inr,
  type Property, type PartnerPayout, type PartnerTicket,
} from "@/data/zones";
...
export const Route = createFileRoute("/partner")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Your properties, occupancy, payouts, and tickets — one workspace." },
    ],
  }),
});
    ...
    <div className="px-4 md:px-8 py-6 md:py-8 max-w-[1300px] mx-auto">
      <header className="mb-6">
        ...
        <p className="text-muted-foreground mt-1 text-sm max-w-2xl">
          Occupancy, monthly payouts, maintenance requests and the visit pipeline — all live, no follow-up calls needed.
        </p>
      </header>
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6">
        <Kpi label="Properties"          value={String(props.length)}                                sub={`${totalBeds} beds total`} icon={Building2} tone="text-primary" />
        <Kpi label="Occupancy"           value={`${Math.round((totalOcc/Math.max(1,totalBeds))*100)}%`} sub={`${totalOcc}/${totalBeds} beds`} icon={TrendingUp} tone="text-success" />
        <Kpi label="Monthly revenue"     value={inr(totalRev)}                                       sub="gross, current month" icon={IndianRupee} tone="text-foreground" />
        <Kpi label="Next payout"         value={inr(nextPayout)}                                     sub={`${payouts.filter(p=>p.status==="Scheduled").length} invoices scheduled`} icon={Wallet} tone="text-info" />
      </section>
      ...
      <div className="flex gap-1 mb-6 border-b border-border overflow-x-auto -mx-4 px-4 md:mx-0 md:px-0">
        {TABS.map(({ id, label, icon: Icon }) => {
          const active = tab === id;
          const count = id === "tickets" ? openTickets : id === "payouts" ? payouts.filter(p=>p.status==="Scheduled").length : 0;
          return (
            <button
              key={id}
              onClick={() => setTab(id)}
            ...
            >
              ...
              {count > 0 && (
                <span className="ml-1 text-[10px] font-mono px-1.5 py-0.5 rounded bg-primary/15 text-primary">{count}</span>
              )}
            </button>
          );
        })}
        ...
        return (
          <article key={p.id} className="rounded-xl bg-card border border-border p-5">
            <div className="flex items-start gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-1">
                  <span className="text-[10px] font-mono uppercase tracking-widest px-1.5 py-0.5 rounded bg-secondary border border-border">{p.type}</span>
                  <span className="inline-flex items-center gap-1 text-xs"><Star className="h-3 w-3 text-warning fill-warning" /> {p.rating}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-3 mt-2 border-t border-border">
              <div>
                <div className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground">Monthly revenue</div>
                <div className="text-sm font-semibold tabular-nums">{inr(p.monthlyRevenue)}</div>
              </div>
              <Link to="/fly" className="text-[10px] font-mono uppercase tracking-widest text-primary">View activity →</Link>
            </div>
          </article>
        );
      })}
  ...
  return (
    <div className="space-y-6">
      {months.map((m) => {
        ...
        const status = rows.every((r) => r.status === "Paid") ? "Paid" : rows.some((r) => r.status === "On Hold") ? "On Hold" : "Scheduled";
        const statusTone = status === "Paid" ? "text-success" : status === "On Hold" ? "text-destructive" : "text-info";
        return (
          <section key={m} className="rounded-xl bg-card border border-border overflow-hidden">
            <div className="px-5 py-4 border-b border-border flex items-center justify-between">
              <div>
                ...
                <div className={`text-[10px] font-mono uppercase tracking-widest mt-0.5 ${statusTone}`}>{status}</div>
              </div>
              <div className="text-right">
                <div className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground">Net to you</div>
                ...
                return (
                  <div key={r.id} className="px-5 py-3 flex items-center gap-3">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground">{r.status}</div>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="px-5 py-3 border-t border-border bg-muted/20 flex items-center justify-between">
              <span className="text-[11px] text-muted-foreground">All amounts in INR, post-GST.</span>
...
// ============== Tickets ==============
function TicketsTab({ tickets, props, partnerId }: { tickets: PartnerTicket[]; props: Property[]; partnerId: string }) {
  const [draft, setDraft] = useState({ title: "", propertyId: props[0]?.id ?? "", category: "Maintenance" as PartnerTicket["category"], priority: "Med" as PartnerTicket["priority"] });
  ...
  function sendToOps() {
    ...
    createTask({
      ...
      dueAt: Date.now() + (draft.priority === "High" ? 4 : draft.priority === "Med" ? 24 : 72) * 3600_000,
    ...
    <div className="grid lg:grid-cols-[1.5fr_1fr] gap-6">
      <section className="space-y-3">
        ...
        {allTickets.map((t) => {
          ...
          const StatusIcon = t.status === "Resolved" ? CheckCircle2 : t.status === "In Progress" ? Clock : AlertTriangle;
          const statusTone = t.status === "Resolved" ? "text-success" : t.status === "In Progress" ? "text-info" : "text-warning";
          const priorityTone =
          ...
          return (
            <article key={t.id} className="rounded-xl bg-card border border-border p-4">
              <div className="flex items-start gap-3">
                <span className={`shrink-0 mt-0.5 text-[9px] font-mono uppercase tracking-widest px-1.5 py-0.5 rounded border ${priorityTone}`}>{t.priority}</span>
                <div className="flex-1 min-w-0">
                ...
                <div className={`shrink-0 flex flex-col items-end ${statusTone}`}>
                  ...
                  <div className="text-[9px] font-mono uppercase tracking-widest mt-1">{t.status}</div>
                </div>
              </div>
              {assignee && (
                <div className="flex items-center gap-2 mt-3 pt-3 border-t border-border">
                  <Avatar id={assignee.id} size={20} />
                  <span className="text-[11px] text-muted-foreground">Owned by {assignee.name.split(" ")[0]} · {assignee.role}</span>
                </div>
              )}
            </article>
          );
      ...
      <aside>
        <div className="rounded-xl bg-card border border-border p-5 sticky top-20">
          <div className="flex items-center justify-between mb-4">
            ...
            {!composing && (
              <button onClick={() => setComposing(true)} className="inline-flex items-center gap-1 text-[10px] font-mono uppercase tracking-widest text-primary">
          ...
          {!composing ? (
            <p className="text-xs text-muted-foreground">
              Maintenance, billing, tenant disputes, compliance — anything. Your zone leader gets paged in real time.
            </p>
          ) : (
            <div className="space-y-2.5">
              <Field label="Property">
                <select value={draft.propertyId} onChange={(e) => setDraft({ ...draft, propertyId: e.target.value })} className="h-9 w-full px-2 bg-background border border-border rounded-md text-sm">
                  {props.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
                </select>
              </Field>
              <Field label="Title">
                <input value={draft.title} onChange={(e) => setDraft({ ...draft, title: e.target.value })} placeholder="e.g. AC not cooling — Room 102" className="h-9 w-full px-2 bg-background border border-border rounded-md text-sm" />
              </Field>
              <div className="grid grid-cols-2 gap-2">
                <Field label="Category">
                  <select value={draft.category} onChange={(e) => setDraft({ ...draft, category: e.target.value as PartnerTicket["category"] })} className="h-9 w-full px-2 bg-background border border-border rounded-md text-sm">
                    <option>Maintenance</option><option>Billing</option><option>Tenant</option><option>Compliance</option><option>Other</option>
                  </select>
                </Field>
                <Field label="Priority">
                  <select value={draft.priority} onChange={(e) => setDraft({ ...draft, priority: e.target.value as PartnerTicket["priority"] })} className="h-9 w-full px-2 bg-background border border-border rounded-md text-sm">
              ...
              <p className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground text-center">{partnerId} · response within 4 working hours</p>
            </div>
          )}
        </div>
      </aside>
...
function VisitsTab({ props }: { props: Property[] }) {
  ...
  const tone = (s: string) => s === "Hot" ? "text-warning border-warning/30 bg-warning/10" : s === "Warm" ? "text-info border-info/30 bg-info/10" : "text-muted-foreground border-border bg-secondary";
    ...
    <div className="grid md:grid-cols-2 gap-4">
      <section className="rounded-xl bg-card border border-border overflow-hidden">
        <div className="px-5 py-4 border-b border-border">
          ...
          <p className="text-[11px] text-muted-foreground mt-0.5">Scheduled by your zone team in the next 48 hours.</p>
        </div>
        <div className="divide-y divide-border">
          {upcoming.map((v) => {
            ...
            return (
              <div key={v.id} className="px-4 py-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium truncate">{v.leadName} → {prop?.name}</div>
                  <div className="text-[11px] text-muted-foreground">{new Date(v.at).toLocaleString([], { weekday: "short", hour: "2-digit", minute: "2-digit" })}</div>
                </div>
                <span className={`text-[9px] font-mono uppercase tracking-widest px-1.5 py-0.5 rounded border ${tone(v.intent)}`}>{v.intent}</span>
              </div>
            );
          })}
        </div>
      ...
      <section className="rounded-xl bg-card border border-border overflow-hidden">
        <div className="px-5 py-4 border-b border-border flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-muted-foreground mt-0.5">For all your properties combined.</p>
          </div>
        </div>
        ...
        <div className="px-5 pb-5">
          <Link to="/fly" className="inline-flex items-center gap-1 text-[10px] font-mono uppercase tracking-widest text-primary">
...
function DocsTab({ props }: { props: Property[] }) {
  const docs = [
    ...
    { name: "Fire NOC — Brook Luxe",     type: "Compliance", status: "Renew by Jul", date: "Expires Jul 2026" },
    { name: "Property tax receipt — Aeris Boys", type: "Compliance", status: "Filed", date: "Mar 2026" },
  ];
  return (
    <div className="grid md:grid-cols-2 gap-6">
          ...
          {docs.map((d, i) => (
            <div key={i} className="px-5 py-3 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[11px] text-muted-foreground">{d.type} · {d.date}</div>
              </div>
              <span className="text-[10px] font-mono uppercase tracking-widest text-success">{d.status}</span>
            </div>
          ))}
        </div>
      </section>
      <section className="rounded-xl bg-card border border-border p-5">
        ...
        <p className="text-xs text-muted-foreground mb-4">Each property has its own folder for inspection reports, photos, and tenant agreements.</p>
        <div className="space-y-2">
          {props.map((p) => (
            <div key={p.id} className="flex items-center justify-between p-2.5 rounded-md border border-border">
              <span className="text-sm font-medium truncate">{p.name}</span>
              <Link to="/" className="text-[10px] font-mono uppercase tracking-widest text-primary">Open folder →</Link>
            </div>
          ))}
        </div>
      </section>
