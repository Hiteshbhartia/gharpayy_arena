...
export const Route = createFileRoute("/war-room")({
  component: () => <RoleGate allow={["leadership", "leader"]}><WarRoom /></RoleGate>,
  ...
  return (
    <div className="px-4 md:px-8 py-6 md:py-8 max-w-[1400px] mx-auto">
      <header className="mb-6">
        <div className="font-mono text-[11px] uppercase tracking-widest text-primary mb-1.5">Daily War Room</div>
        <h1 className="font-display text-2xl md:text-4xl font-semibold tracking-tight">Today's execution snapshot</h1>
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6">
        <StatCard label="Revenue" value={inr(s.totalRevenue)} sub={`${s.totalDeals} deals`} icon={IndianRupee} accent="text-primary" />
        <StatCard label="Calls" value={String(s.totalCalls)} icon={Phone} accent="text-info" />
        <StatCard label="Active leads" value={String(s.totalLeads)} icon={Target} accent="text-warning" />
        <StatCard label="A-players" value={String(s.counts.A)} sub={`B:${s.counts.B} C:${s.counts.C} D:${s.counts.D}`} icon={Flame} accent="text-success" />
      </section>
          ...
          {sorted.map((e, i) => {
            ...
            return (
              <div key={e.id} className="px-4 md:px-5 py-3 flex items-center gap-3">
                ...
                <div className="w-12 text-right font-mono text-sm font-semibold">{e.performance}</div>
                {e.flags.length > 0 && (
